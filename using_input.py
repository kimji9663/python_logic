# 파이썬에서 실행 시 키보드로 값 입력받기 : input() 함수 사용 테스트
# 입력값 기록할 변수 = input('입력을 위한 메세지 문장')

# 입력을 위한 메세지 없이 실행
# num = input('숫자를 입력하세요.')
# print('num :', num, type(num)) # 입력값의 타입은 모두 문자열

# 파이썬의 input() 함수로 입력 들어오는 값은 모두 문자형(str) 이다.
# print(num + 100) # 숫자와 문자는 계산할 수 없음

# 입력된 숫자형문자를 숫자형으로 변환하고자 한다면 형변환함수 사용
# 정수로 변환 : int('정수형문자')
# 실수로 변환 : float('실수형문자')

# inum = int(num)
# print('inum :', inum, type(inum)) # 입력값의 타입은 모두 문자열
# print(inum + 100)
...
# 사용 2 : f'str'(format string) 사용 => 변수를 적용하려면 : f출력문장 {변수명}
print(f'더하기 결과 : {first} + {second} = {first + second}')

# 사용 3 : format() 함수 이용 => 주의 : 순서와 갯수 반드시 일치해야 함
...
# {1:.2f} : 1번째 인덱스 값을 소숫점(float) 아래 둘째 자리까지 출력

print(f'{first} ** 3 = {first ** 3}') # first의 3제곱근
